...
task_out = []
task_in=[]
fi = open("case_test/case_2/input.txt", "r")
fo = open("case_test/case_2/output.txt", "r")
...
for i in stud_dic:
    ...
    if len(stud_dic[i]) != 0:
        inp = "python test_file/" + i + "/task_2.py"
        for date in task_in:
   ...
   # print(ls_answer)
    ...
    if len(ls_answer)==0:
        print(i, "No file")
        sheet['C' + str(ex_i)].value = "No file"

    else:
        print(i, count*5)
        sheet['C' + str(ex_i)].value = count*5
    ls_answer = []
wb.save('resalt.xlsx')